...
```python
df.columns = ['year', 'rank', 'comapny', 'revenue', 'profit']
```
...
print(patient_id, 'weight in kilogrmas:', weight_kg)
```

    inflam_001 weight in kilogrmas: 60.3
...
```python
binomial_name = "Drosophila melangaster"
...
def fahr_to_celsius(temp):
    ...
    #a new variable. This code does the same thinf as the previous function but it is more
...
```python
print('originial min, mean and max are:', numpy.amin(data), numpy.mean(data), numpy.amax(data))
...
```
    ...
    originial min, mean and max are: 0.0 6.14875 20.0
...
```python
def offset_mean(data, target_mean_value):
    """Return a new arrary containing the original data
...
```
    ...
    <ipython-input-7-0d0737626d8d> in normalize_rectangle(rect)
          5     (x0, y0) and (x1, y1) define the lower left and upper right corners
          6     of the rectangle respectively."""
...
input_file_name = input("Enter the name of the input fsata files:")
```

    Enter the name of the input fsata files: sumo.txt
...
```
    ...
    <ipython-input-6-752939e09171> in <module>
          5     codon = rna_sequence[i:i+3]
          6     if len(codon) == 3:
    ----> 7         amino_acid = codon_table[codon]
          8         if amino_acid == "*":
          9             break
